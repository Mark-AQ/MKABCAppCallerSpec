...
project_name = 'LiveInPeace'    # 项目名称
archive_workspace_path = '/Users/mark/Desktop/Test/XXX'    # 项目路径
export_directory = 'archiveipa'    # 输出的文件夹
ipa_download_url = 'https://www.pgyer.com/xLBp'  # 蒲公英的APP地址

# 蒲公英账号USER_KEY、API_KEY
USER_KEY = '7df604c31db77aab7b3bc4b0c04a9250'
API_KEY = 'a26cc141513e2db86a762ac769ccc173'

from_address = '694876940@qq.com'    # 发送人的地址
password = 'aq430620'  # 邮箱密码换成他提供的16位授权码
to_address = '694876940@qq.com'    # 收件人地址,可以是多个的
smtp_server = '694876940.qq.com'    # 因为我是使用QQ邮箱..
...
class AutoArchive(object):
    ...
    def archive(self):
        ...
        # 删除之前的文件
        subprocess.call(['rm', '-rf', '%s/%s' %
                        (archive_workspace_path, export_directory)])
        time.sleep(1)
        # 创建文件夹存放打包文件
        subprocess.call(['mkdir', '-p', '%s/%s' %
                        (archive_workspace_path, export_directory)])
        time.sleep(1)
    ...
    def export(self):
        ...
        if export_result_code != 0:
            ...
            subprocess.call(['rm', '-rf', '%s/%s.xcarchive' %
                            (archive_workspace_path, export_directory)])
            self.upload('%s/%s/%s.ipa' %
                        (archive_workspace_path, export_directory, project_name))
